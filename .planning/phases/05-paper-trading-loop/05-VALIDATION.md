...
| **Estimated runtime** | fast loop ~25s; full suite ~85s (Phase 5 adds ~9 test files, expanded post plan-checker revision) |
...
| 05-01/T2 | 05-01 | 1 | PAPER-03, PAPER-05 | T-05-01, T-05-13 | Deterministic order_ref; persist-before-submit lifecycle; date-independent get_unresolved_orders/find_unresolved_match; get_pending_order_qty excludes pending_submit; retire idempotency | unit | `python -m pytest tests/test_idempotency.py tests/test_paper_ledger.py -x -q` | ❌ W0 | ⬜ pending |
...
| 05-06/T2 | 05-06 | 4 | PAPER-01 | T-05-03, T-05-05, T-05-13 | Gate->sizer->round->HEAL-CHECK(before halt)->halt-gate->persist->submit, no bypass, date-independent crash-safe (BLOCKER 1) | integration (mocked ib_async) | `python -m pytest tests/test_entry_pipeline.py -x -q` | ❌ W0 | ⬜ pending |
...
| 05-07/T2 | 05-07 | 5 | PAPER-07 | T-05-13, T-05-14 | Multi-day BLOCKER 1 heal scenario (crash->halt->clear->next-day heal); composed real-breaker-trip->zero-orders proof (W4); full suite green | integration + full suite | `python -m pytest -q` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/test_idempotency.py` — covers PAPER-03, incl. `find_unresolved_match` date-independent matching (05-01, BLOCKER 1)
- [ ] `tests/test_paper_ledger.py` — covers PAPER-05, incl. pending_submit lifecycle, `get_unresolved_orders`, `heal_order`, `get_pending_order_qty` exclusion (05-01, BLOCKER 1)
...
- [ ] `tests/test_entry_pipeline.py` — covers PAPER-01, heal-before-halt ordering, date-independent heal (05-06, BLOCKER 1)
- [ ] `tests/test_paper_daily_report.py` — covers PAPER-07 daily report section incl. Manual Interventions tally (05-07, W2)
- [ ] `tests/test_recovery.py` — covers PAPER-07 multi-day crash-safety/heal (05-07, BLOCKER 1), composed breaker-trip->zero-orders (05-07, W4)
